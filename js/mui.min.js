...
function getType(id){
	var str = new Array('地库拖车','困境救援 ','路修救援 ','车辆托运 ','救援系列');
	return str[id-1];

}
function getStatus(status){
	var str = new Array('等待接单','司机接单成功 ','司机取消订单 ','用户取消订单 ','挂账中','交易完成','等待用户付款','申请开票','司机到达起始位置');
	return str[status];
}
function typeToName(name,infoData,sum){
	//价格表
	var info = JSON.parse(infoData.content);
	//是否在同一城市
	var isincity = infoData.inCity;
	//救援名称
	var typeName = name;
	//救援距离
	var sumData = sum;
	//救援所需要的金额
	var sumMoney="";
	if(typeName.indexOf("救援系列")>-1){
		var qibu = parseInt(info['救援系列起步公里']);
		//如果是救援系列
		if(sumData<=qibu){
			//未超出范围20KM
			console.log(qibu+"->>"+sumData+"->>"+info['救援系列起步价'])
			sumMoney = parseInt(info['救援系列起步价']);
		}else{
			//超出范围
			sumMoney = parseInt(info['救援系列起步价'])+parseInt(parseInt(sumData)-parseInt(20))*parseInt(info['救援系列超出']);
		}
		return sumMoney;
	}else if(typeName.indexOf("地库拖车")>-1){
		//地库拖车
		sumMoney = parseInt(info['地库拖车']);
		return sumMoney;
	}else if(typeName.indexOf("困境救援")>-1){
		//困境救援
		sumMoney = parseInt(info['困境救援']);
		return sumMoney;
	}else if(typeName.indexOf("路修救援")>-1){
		//路修救援
		var qibu = parseInt(info['路修救援起步公里']);
		//如果是救援系列
		if(sumData<=qibu){
			//未超出范围20KM
			sumMoney = parseInt(info['路修救援起步价']);
		}else{
			//超出范围
			sumMoney = parseInt(info['路修救援起步价'])+parseInt(parseInt(sumData)-parseInt(20))*parseInt(info['路修救援超出']);
		}
		return sumMoney;
	}else if(typeName.indexOf("车辆托运")>-1){
		if(isincity){
			//城市内
			sumMoney = parseInt(info['车辆托运城市以内']);
		}else{
			//城市之间	
			sumMoney = parseInt(info['车辆托运城市之间'])+parseInt(sumData)*parseInt(info['车辆托运公里价']);
		}
		return sumMoney;
	}
}
...
function $ajax(param){
	mui.plusReady(function() {
		...
		var method=param.type?param.type:"POST";
		var url =param.url;
		if(url.indexOf('http')!=0)url=rootpath+url;
		xhr.open(method, url );
		xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
		var data="&";
		for(var key in param.data){
			data+="&"+key+"="+param.data[key];
		}
